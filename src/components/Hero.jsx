...
function CodeCard() {
  return (
    <div className="glass-card rounded-2xl overflow-hidden font-mono text-[13px] w-72 leading-6">
      {/* macOS window chrome */}
      ...
      <div className="p-5 space-y-0.5">
        ...
        <p className="pl-5"><span className="text-primary">],</span></p>
        <p className="pl-5">
...
export default function Hero() {
  ...
  useEffect(() => {
    const onScroll = () => {
      if (contentRef.current) {
        contentRef.current.style.transform = `translateY(${window.scrollY * 0.35}px)`
        contentRef.current.style.opacity = `${1 - window.scrollY / 500}`
      }
    }
  ...
  return (
    <section className="relative flex items-center justify-center min-h-screen px-6 overflow-hidden">
      ...
      {/* Blobs */}
      <div className="absolute top-1/3 left-1/4 w-80 h-80 bg-cyan-600/20 rounded-full blur-3xl animate-blob pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-blue-700/15 rounded-full blur-3xl animate-blob blob-delay pointer-events-none" />

      {/* Content — parallax on scroll */}
      <div ref={contentRef} className="relative z-10 w-full max-w-5xl mx-auto grid lg:grid-cols-[1fr_auto] lg:gap-16 items-center">

        {/* Left: text */}
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left">
          <div className="animate-fade-up flex items-center gap-2 mb-5 px-3 py-1.5 rounded-full glass-card" style={{ borderColor: 'rgba(74,222,128,0.25)' }}>
          ...
          <h1 className="animate-fade-up delay-2 text-6xl md:text-8xl font-extrabold tracking-tight leading-none text-primary mb-6">
            Oscar
            <br />
      ...
      {/* Scroll hint */}
      <div className="absolute bottom-10 z-10 flex flex-col items-center gap-2 animate-bounce pointer-events-none">
        <span className="text-faint text-xs tracking-widest">SCROLL</span>
        <div className="w-px h-8 bg-linear-to-b from-current to-transparent opacity-20" />
      </div>
    </section>
  )
}
